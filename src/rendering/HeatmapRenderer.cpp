...
namespace tfv
{
                               ...
                               int panX, int panY, float scale)
    {
        ...
        for(const auto& visual : roadNetwork->segments())
        {
            ...
            SDL_Color color = getColorForCongestion(congestionLevel);

            // Apply opacity
            color.a = static_cast<Uint8>(m_opacity * 255);
    ...
    void HeatmapRenderer::setColorScheme(SDL_Color lowColor, SDL_Color mediumColor,
                                         SDL_Color highColor)
    {
        m_lowColor = lowColor;
    ...
    SDL_Color HeatmapRenderer::getColorForCongestion(float level) const
    {
        ...
        if(level < 0.5f)
        {
            ...
            float t = level / 0.5f;
            result.r = static_cast<Uint8>(m_lowColor.r + t * (m_mediumColor.r - m_lowColor.r));
            result.g = static_cast<Uint8>(m_lowColor.g + t * (m_mediumColor.g - m_lowColor.g));
            result.b = static_cast<Uint8>(m_lowColor.b + t * (m_mediumColor.b - m_lowColor.b));
            result.a = 255;
        }
        else
        {
            ...
            result.r = static_cast<Uint8>(m_mediumColor.r + t * (m_highColor.r - m_mediumColor.r));
            result.g = static_cast<Uint8>(m_mediumColor.g + t * (m_highColor.g - m_mediumColor.g));
            result.b = static_cast<Uint8>(m_mediumColor.b + t * (m_highColor.b - m_mediumColor.b));
            result.a = 255;
        }
