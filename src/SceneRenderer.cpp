...
namespace tfv
{
    ...
    void RoadRenderer::draw(const RoadNetwork* net)
    {
        ...
        for(const auto& s : segs)
        {
            ...
            m_r->setDrawColor(140, 140, 140, 255);
    ...
    {
        // check if net is not null or empty
        if(!net || net->segments().empty())
            return;
        const auto& segs = net->segments();
        for(const auto& [id, v] : vehicles)
        {
            if(v.segment >= segs.size())
                continue;
            const auto& s = segs[v.segment];
            float x1 = static_cast<float>(s.x1), y1 = static_cast<float>(s.y1);
            float x2 = static_cast<float>(s.x2), y2 = static_cast<float>(s.y2);
            float dx = x2 - x1, dy = y2 - y1;
            float len = std::sqrt(dx * dx + dy * dy);
            ...
            float segLen = net->segmentLength(v.segment);
            float t = segLen > 0.f ? std::fmod(v.position, segLen) / segLen
                                   : 0.f; // fraction along the segment
            float wx = x1 + ux * (segLen * t);
            float wy = y1 + uy * (segLen * t);
            int sx = static_cast<int>(wx * m_scale) + m_panX;
            int sy = static_cast<int>(wy * m_scale) + m_panY;
            m_r->setDrawColor(0, 200, 0, 255);
            if(m_scale < 2.0f)
            {
                m_r->drawPoint(sx, sy);
            }
            else
            {
                ...
                // TODO: draw arrowhead for direction
            }
        }
    }
} // namespace tfv